...
int motion_attack = 0;		//çUåÇâ¡éZíl
int motion_y = 0;			//à⁄ìÆéûÇÃçÇÇ≥
bool warpFlg = false;		//à⁄ìÆÉtÉâÉO
int warp_num = 0;			//0:à⁄ìÆäJén 1:îΩëŒë§Ç÷ 2:à⁄ìÆäÆóπ
...
bool Bomb_Flg2[3] = { false, false, false };//íµÇÀï‘Ç¡ÇΩîöíe
int Bomb_count[3] = { 0 };
int bomb_num = 0;
		...
		if (Bomb_Y[0] <= 9 * CHIPSIZE) {//ìäâ∫
			...
			if (g_map.gimmickData[(Bomb_Y[0] / CHIPSIZE) + 1][Bomb_X[0] / CHIPSIZE] == BOSS_G_3) {//íeóÕÇ»ÇÁíµÇÀï‘Ç∑
				...
				if (Bomb_count[0]-- > 0) {//îöîjÉJÉEÉìÉg
					...
					DrawExtendGraph(Bomb_X[0] - 32, Bomb_Y[0] - 64, Bomb_X[0] + 160, Bomb_Y[0] + 64, g_img.BombEff, TRUE);//îöî≠îÕàÕ
		...
		if (Bomb_Y[1] <= 9 * CHIPSIZE) {
			...
			if (g_map.gimmickData[(Bomb_Y[1] / CHIPSIZE) + 1][Bomb_X[1] / CHIPSIZE] == BOSS_G_3) {
				...
				if (Bomb_count[1]-- > 0) {
					...
					DrawExtendGraph(Bomb_X[1] - 32, Bomb_Y[1] - 64, Bomb_X[1] + 160, Bomb_Y[1] + 64, g_img.BombEff, TRUE);//îöî≠îÕàÕ
		...
		if (Bomb_Y[2] <= 9 * CHIPSIZE) {
			...
			if (g_map.gimmickData[(Bomb_Y[2] / CHIPSIZE) + 1][Bomb_X[2] / CHIPSIZE] == BOSS_G_3) {
				...
				if (Bomb_count[2]-- > 0) {
					...
					DrawExtendGraph(Bomb_X[2] - 32, Bomb_Y[2] - 64, Bomb_X[2] + 160, Bomb_Y[2] + 64, g_img.BombEff, TRUE);//îöî≠îÕàÕ
			...
			if (Bomb_Y[bomb_num] < 3 * CHIPSIZE) {//îöíeç~â∫
				...
				if (Bomb_count[bomb_num]-- > 0) {
					...
					if (g_boss[1].count == 0) {
						...
						DrawExtendGraph(Bomb_X[bomb_num] - 32, Bomb_Y[bomb_num] - 64, Bomb_X[bomb_num] + 160, Bomb_Y[bomb_num] + 64, g_img.BombEff, TRUE);//îöî≠îÕàÕ
...
void boss2_Init(){
	...
	motion_attack = 0;		//çUåÇâ¡éZíl
	motion_y = 0;			//à⁄ìÆéûÇÃçÇÇ≥
	warpFlg = false;		//à⁄ìÆÉtÉâÉO
	warp_num = 0;			//0:à⁄ìÆäJén 1:îΩëŒë§Ç÷ 2:à⁄ìÆäÆóπ
	...
	for (int i = 0; i < 3; i++) {
		...
		Bomb_speed[i] = 0;
		Bomb_Flg[i] = false;
		Bomb_Flg2[i] = false;//íµÇÀï‘Ç¡ÇΩîöíe
		Bomb_count[i] = 0;
	}
	bomb_num = 0;
}